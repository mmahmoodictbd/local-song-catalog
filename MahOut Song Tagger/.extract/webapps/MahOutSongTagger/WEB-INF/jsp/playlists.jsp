	...
	<table class="table table-condensed">
		...
		<tbody>
			<c:forEach var="tag" varStatus="status" begin="0"
				...
				<tr>
					<td><a
						href="${contextPath}/searchSong?searchString=${tag}&searchExcludeFileName=on&searchExcludeTags=on&searchExcludeFilePath=on&searchExcludeLyrics=on"><i
							class="icon-reorder"></i> ${tag}</a></td>
					...
					<td><a class="confirm-save"
						href="${contextPath}/savePlaylistToFile?playlist=${tag}"><i
							class="icon-save"></i></a></td>
					...
					<c:choose>
						<c:when test="${not empty tags[status.index+1]}">
							<td><a
								href="${contextPath}/searchSong?searchString=${tags[status.index+1]}&searchExcludeFileName=on&searchExcludeTags=on&searchExcludeFilePath=on&searchExcludeLyrics=on"><i
									class="icon-reorder"></i> ${tags[status.index+1]}</a></td>
							<td><a class="confirm-edit" href="#edit" data-toggle="modal"
							...
							<td><a class="confirm-delete" href="#delete"
								data-toggle="modal" data-tagname="${tags[status.index+1]}"><i
									class="icon-trash"></i></a></td>
							<td><a class="confirm-save"
								href="${contextPath}/savePlaylistToFile?playlist=${tags[status.index+1]}"><i
									class="icon-save"></i></a></td>
						</c:when>
						<c:otherwise>
							<td></td>
					...
					<c:choose>
						<c:when test="${not empty tags[status.index+2]}">
							<td><a
								href="${contextPath}/searchSong?searchString=${tags[status.index+2]}&&searchExcludeFileName=on&searchExcludeTags=on&searchExcludeFilePath=on&searchExcludeLyrics=on"><i
									class="icon-reorder"></i> ${tags[status.index+2]}</a></td>
							<td><a class="confirm-edit" href="#edit" data-toggle="modal"
							...
							<td><a class="confirm-delete" href="#delete"
								data-toggle="modal" data-tagname="${tags[status.index+2]}"><i
									class="icon-trash"></i></a></td>
							<td><a class="confirm-save"
								href="${contextPath}/savePlaylistToFile?playlist=${tags[status.index+2]}"><i
									class="icon-save"></i></a></td>
						</c:when>
						<c:otherwise>
							<td></td>
